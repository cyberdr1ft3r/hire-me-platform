import type { TaskDetail as TaskRecord, TaskPriority, TaskStatus } from '@hire-me/contracts';
import {
  useEffect,
  useId,
  useRef,
 ...
 * close button close it, and focus returns to the card that opened it. The
 * board behind it is inert while it is open.
 */
export function TaskDrawer({
  ...
  useEffect(() => {
    ...
    return () => {
      if (target) {
        document.querySelector<HTMLElement>(`[data-task-card="${target}"]`)?.focus();
      }
    };
  }, []);
          ...
          <div>
            ...
            <dd>
              {activeAssignees.length
                ? activeAssignees.map((assignment) => assignment.userDisplayName).join(', ')
                : t('task.detail.noAssignees')}
            </dd>
          </div>
          <div>
