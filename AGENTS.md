...
The saved environment encodes this: `install` refreshes dependencies, builds the shared `@hire-me/contracts` package, and generates the Prisma client under Node 24; `start` brings up PostgreSQL, applies migrations, runs the idempotent seed, and launches `pnpm dev` (API on `127.0.0.1:3000`, web on `127.0.0.1:5173`). Building `@hire-me/contracts` before `pnpm dev` avoids a cold-boot race where `apps/api` (tsx watch) can start before `packages/contracts/dist` exists. Everything else follows the root `README.md`.
