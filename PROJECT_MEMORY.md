...
- Generation never truncates or substitutes authoritative text. PDF output uses `pdfkit` with repository-owned SIL Open Font License Noto faces, `fontkit` shaping, and `bidi-js` UAX #9 ordering, so Latin, Greek, Cyrillic, and Arabic all render correctly, including mixed-direction lines. A script no bundled face covers fails closed; the Word output carries full Unicode.
