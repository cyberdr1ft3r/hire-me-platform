...
- Templates are a code-owned registry of TypeScript functions over a neutral, data-only renderable document: no template language, no HTML, no evaluation, no uploaded template, and no remote fetch. Renderers are pure JavaScript (`pdf-lib`, `docx`) and need no native binary, browser, office suite, or shell.
- Every generated version records the exact template id, template version, language, output family, checksum, and a `sourceSnapshotSha256` fingerprint of the authoritative fields it rendered. The source is re-read inside the publishing transaction and the fingerprint recomputed, so a file rendered from a stale mutable record is never committed.
- Generation never truncates or substitutes authoritative text. PDF output wraps across lines and pages and covers the full WinAnsi repertoire; text a PDF standard font cannot encode fails closed rather than being corrupted, and the Word output carries full Unicode.
