...
Mission-candidate responses are shaped by the caller's effective permissions. Internal notes require `mission_candidate_notes:view`; live candidate compensation requires `candidate_compensation:view`; live candidate consent requires `candidate_consent:view`. Linking a candidate to a mission does not make the candidate client-visible. Client visibility starts only through the explicit presentation action, and future client-facing APIs must continue to exclude internal notes, confidential scores, other missions, and internal history.
