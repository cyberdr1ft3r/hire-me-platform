...
- Issue #19 is implemented on branch `feat/recruitment-missions` and is ready for draft PR review.
...
- Mission updates, status changes, closure, archival, assignment writes, assignment archival, and lead replacement share the parent `RecruitmentMission` PostgreSQL row lock.
...
`pnpm test:db` was rerun outside the filesystem sandbox when esbuild needed access to local config paths. Safe synthetic auth environment values were used for the local test process only because the local `.env` did not contain auth secrets. PostgreSQL used Docker Compose on `127.0.0.1:5432`.
