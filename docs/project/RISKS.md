...
| R-015 | Client CRM row-scope assumptions grant broad organization or contact access before team, assignment, or client-portal scoping exists. | One client, team, or employee could see another party's confidential client or contact data. | Issue #15 seeds broad client CRM permissions only to explicitly authorized internal roles, denies unresolved scoped roles by default, verifies nested contact ownership, gates commercial fields separately, serializes client archival and dependent writes on the parent client row, and tests IDOR/lifecycle/concurrency behavior against PostgreSQL. | Active |
