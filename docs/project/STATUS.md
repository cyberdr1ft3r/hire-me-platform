...
**Current blocker:** ChatGPT D-PUBLIC-01 conformance review of the Issue #73 draft PR.
**Next executable development task:** Review the Issue #73 unit boundary, the read-only legacy treatment, and the deliberate absence of any historical backfill. Keep the PR draft, open, unmerged, and undeployed. R-039 remains untouched.
...
- Tests: contracts 30 → 34 (new `public-applications.test.ts`), web 442 → 452, PostgreSQL 314 → 318. Three existing assertions that pinned the buggy unit were corrected; no test was deleted, skipped, retried, or given a longer timeout.
