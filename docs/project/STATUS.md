...
- English is canonical. French is annotated with `Messages`, so a missing, extra, renamed, or misspelled key fails `pnpm typecheck`. Both failure modes were exercised on this branch before the tests were written. Runtime lookup, interpolation, and plural misuse throw rather than falling back.
...
- Web tests went from 59 to 105. The 46 new tests cover locale resolution and persistence, allow-list rejection, dictionary parity, markup-free values, typed lookup failure, interpolation, `Intl.PluralRules` plurals in both locales, `Intl` number/date/currency formatting with an explicit timezone and explicit `MAD`/`EUR` codes, live language switching, unchanged route and destinations, absence of API calls on switch, sign-out after switching, `documentElement.lang`, and the public/auth boundary.
