...
- The salary expectation crosses one explicit unit boundary (Issue #73, D-066). The browser form holds the amount in normal major currency units, exactly as a person types it, and converts it on the digit string before building the request. The public request field `salaryExpectationCents`, `PublicCandidateApplication.submittedSalaryExpectationCents`, and `Candidate.salaryExpectationCents` are integer minor units for every caller, bounded by the `integer` column maximum, so a direct API caller keeps sending cents and an oversized amount is a request validation failure rather than a database error.
...
Commercial records remain structured source-of-truth data. Issue #38 implements quotations, recruitment/training commercial contracts, purchase orders, and invoices as API-owned structured records with server-calculated totals, same-client/business-context/currency source validation, hidden-ID masking, lifecycle history, audit summaries, commercial-data redaction, historical read preservation after parent archival, and exact contract/PO invoice snapshots. Issued invoices keep immutable line and amount snapshots, and placement-backed invoices require an eligible confirmed `MissionPlacement`; `CLOSED_WITH_RECRUITMENT` does not by itself block invoicing when the placement remains eligible. Payment, partial-payment, overdue-balance, expense, client-balance, revenue, and profitability concepts are not `Document` records merely because they can later be generated as PDF, Word, or Excel outputs, and they remain later scope. Full legal accounting, general ledger, tax declarations, bank reconciliation, and balance-sheet behavior remain unresolved.
